module MonthlyGrantsBaseReport
  ...
  def by_month_report request_ids, params, aggregate_type=:count
    ...
    plot[:axes] = { :xaxis => { :min => 0, :max => i, :ticks => axis, :tickOptions => { :angle => -30 }}, :yaxis => { :min => 0, :tickOptions => {:formatString => aggregate_type == :sum_amount ? "#{I18n.t 'number.currency.format.unit'}%'.0f" : '%d'}}}
