module FluxxProgram
  ...
  PROGRAM_FSA_JOIN_FUNDING_SOURCE_CLAUSE = "funding_source_id in (select id from funding_sources where state in (?))"
  ...
  def self.included(base)
    ...
    base.insta_export do |insta|
      ...
      # We use a custom spreadsheet view to handle the special way the data is presented
      insta.spreadsheet_template = "program_spreadsheet"
  ...
  module ModelClassMethods
    ...
    def all_program_users
      ...
      User.joins(:role_users => :role).where({:user_profile_id => employee_profile.id, :deleted_at => nil, :role_users => {:roles => {:roleable_type => self.name}}}).order('last_name asc, first_name asc').group("users.id").compact
    end
  ...
  module ModelInstanceMethods
    ...
    def load_users role_name=nil
      ...
      user_query = User.joins(:role_users => :role).order('first_name asc, last_name asc').where({:deleted_at => nil, :user_profile_id => (employee_user_profile ? employee_user_profile.id : nil), :test_user_flag => 0, :role_users => {:roleable_id => program_ids, :roles => {:roleable_type => self.class.name}}})
    ...
    def total_pipeline request_types=nil, spending_year=nil
      ...
      total_amount = FundingSourceAllocation.connection.execute(
        ...
        and rfs.funding_source_allocation_id = fsa.id and
                #{program_fsa_join_where_clause}",self.id, self.id, self.id, self.id, FundingSource.approved_states]))
