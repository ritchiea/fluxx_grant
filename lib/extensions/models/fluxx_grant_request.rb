  ...
  module ModelInstanceMethods
        ...
        if self.is_er?
          if program_organization.grants.size > 0 # Is there another grant that already exists
            ...
            if duration_in_months > 12
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id,
                :amount_due => amount_recommended * 0.5, :due_at => grant_agreement_at, :state => tentatively_due_state)
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id,
                :amount_due => amount_recommended * 0.4, :due_at => interim_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'interim_request')
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, 
                :amount_due => amount_recommended * 0.1,:due_at => final_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'final_request')
            else
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id,  
                :amount_due => amount_recommended * 0.9, :due_at => grant_agreement_at, :state => tentatively_due_state)
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, 
                :amount_due => amount_recommended * 0.1,:due_at => final_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'final_request')
            end
          else
            ...
            if duration_in_months > 12
              ...
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, 
                :amount_due => amount_recommended * 0.6, :due_at => grant_agreement_at, :state => tentatively_due_state)
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, 
                :amount_due => amount_recommended * 0.3,:due_at => interim_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'interim_request')
              request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id,
                :amount_due => amount_recommended * 0.1,:due_at => final_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'final_request')
            end
          end
          ...
          if duration_in_months > 12
            request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, :amount_due => amount_recommended * 0.5, 
              :due_at => grant_agreement_at, :state => tentatively_due_state)
            request_transactions << RequestTransaction.new(:request => self, :created_by_id => self.updated_by_id, :updated_by_id => self.updated_by_id, :amount_due => amount_recommended * 0.5, 
              :due_at => interim_request_document.due_at, :state => tentatively_due_state, :request_document_linked_to => 'interim_request')
          else
    ...
    def generate_grant_reports
      ...
      if report_style == 'simple'
        ...
        # Interim Reports
        if duration_in_months > 12
          request_reports << RequestReport.new(:request => self, :due_at => (grant_begins_at + 10.months).next_business_day, :report_type => RequestReport.interim_budget_type_name, :state => new_state)
          request_reports << RequestReport.new(:request => self, :due_at => (grant_begins_at + 10.months).next_business_day, :report_type => RequestReport.interim_narrative_type_name, :state => new_state)
        elsif new_grantee
          request_reports << RequestReport.new(:request => self, :due_at => (grant_begins_at + (grant_ends_at - grant_begins_at) / 2).next_business_day, :report_type => RequestReport.interim_budget_type_name, :state => new_state)
          request_reports << RequestReport.new(:request => self, :due_at => (grant_begins_at + (grant_ends_at - grant_begins_at) / 2).next_business_day, :report_type => RequestReport.interim_narrative_type_name, :state => new_state)
        end
        ...
        if self.is_er?
          ...
          request_reports << RequestReport.new(:request => self, :due_at => (grant_ends_at + 2.month).next_business_day, :report_type => RequestReport.final_budget_type_name, :state => new_state)
          request_reports << RequestReport.new(:request => self, :due_at => (grant_ends_at + 2.month).next_business_day, :report_type => RequestReport.final_narrative_type_name, :state => new_state)
        end
        final_request_document = request_reports.last

        # Eval Reports
        eval_request_document = RequestReport.new(:request => self, :due_at => (final_request_document.due_at + 1.month).next_business_day, :report_type => RequestReport.final_eval_type_name, :state => new_state)
