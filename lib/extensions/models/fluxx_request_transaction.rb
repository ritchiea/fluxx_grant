module FluxxRequestTransaction
  ...
  def self.included(base)
    ...
    base.insta_template do |insta|
      ...
      insta.add_methods [:amount_due, :due_at, :condition, :paid_at, :amount_paid, :request]
  ...
  module ModelInstanceMethods
    ...
    def update_state_by_payment_type
      unless changed_attributes['state'] || promotion_event
        paid_state = RequestTransaction.all_states_with_category('paid').first
      
        if payment_type == 'Credit Card' && state != paid_state
          update_attributes(:state => paid_state, :paid_at => (self.paid_at || Time.now), :amount_paid => amount_due) 
        end
      end
    end
