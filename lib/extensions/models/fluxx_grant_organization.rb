module FluxxGrantOrganization
  ...
  def self.included(base)
    ...
    base.insta_export do |insta|
      ...
      insta.headers = ['name', ['Date Created', :date], ['Date Updated', :date], 'street_address', 'street_address2', 'city', 'state_name',
                  'country_name', 'postal_code', 'phone', 'other_contact', 'fax', 'email', 'url', 'blog_url', 'twitter_url', 'acronym', 'tax_class', 'ein']
      insta.spreadsheet_cells = [:name, :created_at, :updated_at, :street_address, :street_address2, :city, [:geo_state, :name], [:geo_country, :name],
                  :postal_code, :phone, :other_contact, :fax, :email, :url, :blog_url, :twitter_url, :acronym, :tax_class, :tax_id]
      insta.sql_query = "organizations.name, organizations.created_at, organizations.updated_at, street_address, street_address2, city, geo_states.name state_name,
