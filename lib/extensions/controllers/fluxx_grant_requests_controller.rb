  ...
  def self.included(base)
    ...
    base.insta_index Request do |insta|
      ...
      insta.summary_view do |format|
        format.html do |triple|
          controller_dsl, outcome, default_block = triple
          query = "SELECT SUM(IF(r.amount_recommended, r.amount_recommended, r.amount_requested)) AS amount, AVG(DATEDIFF(CURDATE(), r.created_at)) as days, AVG(IF(r.amount_recommended, r.amount_recommended, r.amount_requested)) AS average, COUNT(r.id) AS count FROM requests r WHERE r.id IN (?)"
          ids=  @models.map(&:id)
          ...
          query = "SELECT sum(IF(r.amount_recommended, r.amount_recommended, r.amount_requested)) as amount, count(r.id) as count, r.state AS state FROM requests r WHERE r.id IN (?) and r.state not in (?) group by r.state"
