    ...
    base.insta_index Request do |insta|
      ...
      insta.summary_view do |format|
        format.html do |triple|
          ...
          @pipeline = []
          query = "SELECT sum(IF(r.amount_recommended, r.amount_recommended, r.amount_requested)) as amount, count(r.id) as count, r.state AS state FROM requests r WHERE r.id IN (?) group by r.state"
