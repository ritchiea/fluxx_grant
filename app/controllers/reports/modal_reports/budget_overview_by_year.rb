class BudgetOverviewByYear < ActionController::ReportBase
  ...
  def compute_plot_data controller, index_object, params, report_vars, models
    ...
    FundingSourceAllocation.build_temp_table do |temp_table_name|
      ...
      query = "SELECT SUM(rfs.funding_amount) AS amount, fsa.program_id FROM requests r, request_funding_sources rfs, #{temp_table_name} fsa
        WHERE rfs.request_id = r.id and fsa.id = rfs.funding_source_allocation_id AND
        #{always_exclude} AND granted = 1 AND grant_agreement_at >= ? AND grant_agreement_at <= ? AND fsa.program_id IN (?) GROUP BY fsa.program_id"
      total_grant_allocations = ReportUtility.query_map_to_array([query, start_date, stop_date, program_ids], program_ids, :program_id, :amount)
      ...
      budgeted.each_index{|i| budgeted[i] = 0 unless budgeted[i]; budgeted[i] -= ((pipeline[i] || 0) + (total_granted[i] || 0)) }

      paid.each_index{|i| paid[i] = 0 unless paid[i]; paid[i] -= ((budgeted[i] || 0) + (pipeline[i] || 0) + (total_granted[i] || 0)) }

      hash[:data] = ReportUtility.convert_bigdecimal_to_f_in_array [pipeline, total_granted, total_grant_allocations, budgeted, paid ]
      ...
      hash[:series] = [ {:label => "Pipeline", :renderer => "$.jqplot.BarRenderer"}, {:label => "Granted (By Program)", :renderer => "$.jqplot.BarRenderer"}, {:label => "Granted (By Allocation)", :renderer => "$.jqplot.BarRenderer"}, {:label => "Budgeted"}, {:label => "Paid"} ]
  ...
  def report_legend controller, index_object, params, report_vars, models
    ...
    categories = ["Pipeline", "Granted (By Program)", "Granted (By Allocation)", "Budgeted", "Paid"]
        ...
        case program
          ...
          when "Granted (By Allocation)"
            query = "SELECT SUM(rfs.funding_amount) AS amount, count(distinct r.id) AS count FROM requests r, request_funding_sources rfs, #{temp_table_name} fsa
              WHERE rfs.request_id = r.id and fsa.id = rfs.funding_source_allocation_id AND
              #{always_exclude} AND granted = 1 AND grant_agreement_at >= ? AND grant_agreement_at <= ? AND fsa.program_id IN (?) AND type = ?
              "
              
            grant = [query, start_date, stop_date, program_ids, 'GrantRequest']
            fip = [query, start_date, stop_date, program_ids, 'FipRequest']
            card_filter ="utf8=%E2%9C%93&request%5Bdate_range_selector%5D=funding_agreement&request%5Brequest_from_date%5D=#{start_date_string}&request%5Brequest_to_date%5D=#{stop_date_string}&request%5B2has_been_rejected%5D=&request%5Bsort_attribute%5D=updated_at&request%5Bsort_order%5D=desc&request[funding_source_allocation_program_id][]=" + program_ids.join("&request[funding_source_allocation_program_id][]=")
          when "Paid"
