class ProgramInitiativeBudgetReport < ActionController::ReportBase
  ...
  def compute_document_data controller, show_object, params, report_vars, models
    ...
    Request.find_by_sql(["select requests.id id, if(type = 'GrantRequest', organizations.name, fip_title) budget_report_grant_title, requests.type from requests left outer join organizations on requests.program_organization_id = organizations.id where requests.id in (?)", request_data.keys]).each do |req|
      if request_data[req.id]
        request_data[req.id][:grant_title] = req.budget_report_grant_title
